molass\_legacy package
======================

.. automodule:: molass_legacy
   :members:
   :undoc-members:
   :show-inheritance:

Submodules
-----------

.. toctree::
   :maxdepth: 4

   molass_legacy.ATSAS
   molass_legacy.Alsaker
   molass_legacy.AutorgKek
   molass_legacy.Baseline
   molass_legacy.Batch
   molass_legacy.BoundedLRF
   molass_legacy.CFSD
   molass_legacy.CharFunc
   molass_legacy.Conc
   molass_legacy.DENSS
   molass_legacy.DMM
   molass_legacy.DataStructure
   molass_legacy.Decomposer
   molass_legacy.DevelUtils
   molass_legacy.Distance
   molass_legacy.EFA
   molass_legacy.Elution
   molass_legacy.Env
   molass_legacy.Error
   molass_legacy.Estimators
   molass_legacy.ExcelProcess
   molass_legacy.Experiment
   molass_legacy.Extrapolation
   molass_legacy.Factors
   molass_legacy.Global
   molass_legacy.Gmm
   molass_legacy.GuiParts
   molass_legacy.GuinierAnalyzer
   molass_legacy.GuinierTools
   molass_legacy.HdcTheory
   molass_legacy.Hplc
   molass_legacy.ICIS
   molass_legacy.IFT
   molass_legacy.InputProcess
   molass_legacy.Irregular
   molass_legacy.Jupyter
   molass_legacy.KDE
   molass_legacy.KekLib
   molass_legacy.Kratky
   molass_legacy.LRF
   molass_legacy.LoaderProcess
   molass_legacy.MD
   molass_legacy.MXD
   molass_legacy.Mapping
   molass_legacy.Menus
   molass_legacy.Microfluidics
   molass_legacy.ModelParams
   molass_legacy.Models
   molass_legacy.MultiProc
   molass_legacy.ObjectiveFunctions
   molass_legacy.OnTheFly
   molass_legacy.Optimizer
   molass_legacy.PSD
   molass_legacy.Particles
   molass_legacy.Pdb
   molass_legacy.Peaks
   molass_legacy.Pomegranate
   molass_legacy.Prob
   molass_legacy.Processes
   molass_legacy.Protein
   molass_legacy.Python
   molass_legacy.QuickAnalysis
   molass_legacy.RangeEditors
   molass_legacy.Rank
   molass_legacy.ReAtsas
   molass_legacy.Reports
   molass_legacy.RgProcess
   molass_legacy.Rgg
   molass_legacy.SSDC
   molass_legacy.Saxs
   molass_legacy.SaxsTools
   molass_legacy.SecSaxs
   molass_legacy.SecTheory
   molass_legacy.SecTools
   molass_legacy.Selective
   molass_legacy.SerialAnalyzer
   molass_legacy.SimTools
   molass_legacy.Simulative
   molass_legacy.Solvers
   molass_legacy.Stochastic
   molass_legacy.SymPy
   molass_legacy.Synthesizer
   molass_legacy.Test
   molass_legacy.Theory
   molass_legacy.Tools
   molass_legacy.Trials
   molass_legacy.Trimming
   molass_legacy.Tutorials
   molass_legacy.UV
   molass_legacy.V2PropOptimizer
   molass_legacy.Wave
   molass_legacy.ref_codes

Submodules
----------

.. toctree::
   :maxdepth: 4

   molass_legacy.main
