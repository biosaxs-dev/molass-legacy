"""
    ModelParams.GrmParamsSheet.py

    Parameter inspection sheet for GRM (General Rate Model, G1500).
...
class GrmParamsSheet(ParamsSheetBase):
    def __init__(self, parent, params, dsets, optimizer):
        ...
        # split_params_simple returns 8 elements for GRM:
        # [xr_params (nc,), xr_baseparams, rg_params, mapping,
        #  uv_params (nc,), uv_baseparams, mappable_range, grm_colparams]
        (xr_params, xr_baseparams, rgs, mapping,
         uv_params, uv_baseparams, mappable_range,
         grm_colparams) = optimizer.split_params_simple(params)
        ...
        # grm_colparams are appended outside the decomp block
        ...
        for i, name in enumerate(colparam_names):
            ...
            data_list[row_offset][1] = "%g" % grm_colparams[i]
