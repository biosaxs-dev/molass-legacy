...
class BasicOptimizer:
    def __init__(self, dsets, n_components, params_type, kwargs):
        ...
        self.use_K = getattr(params_type, 'use_K', False)  # opt-in; only StcParams sets True
        if self.use_K:
    ...
    def solve(self, init_params, real_bounds=None, niter=100, seed=None, callback=True, method=None,
        ...
        if method == "bh":
            ...
            # Registry-backed solvers (CMA, DE, NSGA2, and future additions)
            # get_solver_instance handles reload + SerialSettings kwargs automatically.
            # To add a new solver: edit Registry.py only.
            try:
                from importlib import reload as _reload
                import molass_legacy.Solvers.Registry
