...
| Jul 2026 | **AI-friendliness: DataSet.jvector alias (#90) and SecEstimator poresize guard (#91)** (`DataSet.py`, `SecEstimator.py`, commit `2c1f419`): Two naming/API friction points discovered while writing `34a_pre_fix_baseline.ipynb`. (1) `DataSet.xr_ex` vs `SerialData.jvector` — added `@property def jvector(self): return self.xr_ex` to `DataSet`. (2) `guess_initial_secparams` crashed with `float / NoneType` when `poresize` not set in SerialSettings — added explicit `ValueError` with message pointing to `set_setting('poresize', <value>)`. Same pattern already worked around in `OptDataSetsProxy.py` with a comment. |
...
| Jul 2026 | **SSD-native `prepare_rg_curve` — three sequential bugs** (`PeakEditor.py`, `DataSet.py`, commits `ea94c36d`, `148e250b`, `0fcca682`): `prepare_rg_curve` was changed to call `SSD(in_folder).trimmed_copy().corrected_copy()` (absolute jv [59, 1557], 1499 frames) instead of `make_ssd_from_sd(self.sd).trimmed_copy()` (0-based jv, 645 frames). This introduced three bugs in sequence: **(1) DataSet.xr_index AttributeError** — `xr_index` init block was accidentally stranded inside `jvector` property body after `return self.xr_ex` → unreachable dead code → `DataSet.copy()` crashed. Fix (commit `148e250b`): moved block back to `__init__`. **(2) qvector/xrD shape mismatch** — `self.qvector` (from `self.sd.qvector` = legacy 995 q-pts) ≠ `xrD.shape[0]` (from `_lib_dsets` built from full raw data trim, different q-count) → `GuinierDeviation.update_region` crash. Fix (commit `0fcca682`): `construct_optimizer` uses `_ssd_uncorrected.xr.q_values` when `_lib_dsets` active. **(3) UV-XR mapping shift** — `get_pre_recog_mapping_params()` returned legacy `pre_recog.cs.slope/intercept` calibrated for 0-based frame space; with absolute XR frames [59, 1557] the UV components appeared shifted left. Fix (commits `7c12f392`, `2055cbbd`): override in `PeakEditor.get_pre_recog_mapping_params` to return `decomposition.ssd.get_mapping()` (library-computed `MappingInfo`), identical to what `MockEditor` already does. **Verified**: analysis-004 (20230705, EGH 3-comp) GUI SV=75.4, UV heights [0.0145, 0.0493, 0.268] match simulation (34b, SV=76.25) ✓. Remaining gap ~0.85 SV is within tolerance. |
| Jul 2026 | **SSD-native subprocess UV frame-axis length mismatch** (`OptDataSets.py`, `BackRunner.py`, `LegacyBridgeUtils.py`, commits `d06a2c44` molass-legacy / `10d789a` molass-library, then `3b26fd2e` molass-legacy): After the SSD-native `prepare_rg_curve` fix, `ip_uv_elcurve_y.npy` exported 2249 in-process UV-native frames but the subprocess loaded only 645 legacy UV frames into `uv_curve.x`. The `ip_uv_elcurve_y` override then crashed at `InterpolatedUnivariateSpline(uv_curve.x[645], _new_uv_y[2249])` → `ValueError: x and y should have a same length`. Fix: molass-library `prepare_rigorous_folders` now also exports `ip_uv_elcurve_x.npy` (`dsets[2][0].x`). `OptDataSets.get_dsets_impl` loads `ip_uv_elcurve_x.npy` and overrides `uv_curve.x` only when the file exists AND the lengths differ (backward-compatible: old runs without `ip_uv_elcurve_x.npy` are unaffected). No `apply_x_shifts` conflict: `x_shifts.txt` is absent for this run (BackRunner received `x_shifts=None`), so `apply_x_shifts` is not called and the overridden absolute UV frames stay correct. **Second root cause** (commit `3b26fd2e`): `BackRunner.run()` has its own ip_*.npy export block (molass-library#206) that bypasses `prepare_rigorous_folders` in the GUI path. This block was also missing `ip_uv_elcurve_x.npy`. Fix: added `_np.save(..., 'ip_uv_elcurve_x.npy', optimizer.uv_curve.x)` to the BackRunner export block. |

| Jul 2026 | **molass-legacy#94 — rg-curve/ exported with legacy coordinates** (`PeakEditor.py`, commit `d7a20c28`): `prepare_rg_curve()` called `LegacyRgCurve(self.ecurves[1], library_rgcurve)` where `self.ecurves[1].x = [0..644]` (legacy 0-based) but `library_rgcurve.indeces` are in absolute frame space [59..1557]. `LegacyRgCurve` uses `frame_offset = x[0] = 0`, so `rel_indices = indeces - 0 = [59..1557]`, which are mostly outside [0, 644] → masked out → NaN Rg values → wrong slices exported to `rg-curve/`. Subprocess `RgCurveProxy` reads legacy slices (e.g. `slice(136,309)`) on a 1499-length `valid_bool_all` array → indexes buffer region instead of peak region → `get_reconstructed_curve` picks wrong Cxr columns → inflated Guinier deviation → ~0.155 fv gap → ~5.5 SV gap between in-process (75.4) and subprocess (69.9). Fix: use `ssd.xr.get_icurve()` (x = ssd.xr.jv = [59..1557]) wrapped in `_SsdXrCurveAdapter` as the `ecurve` argument to `LegacyRgCurve`. `frame_offset=59`, `rel_indices=indeces-59` → correct Rg placement → correct slices. BackRunner's own rg-curve export is already guarded by `ok.stamp` check and skips when PeakEditor has already exported. |

| Jul 2026 | **SSD-native subprocess diff_spline domain mismatch — init SV gap** (`BackRunner.py`, commit `988b2174`): After all ip_*.npy overrides, subprocess init SV (69.9) was still 5.5 SV below in-process draw_scores SV (75.4) for the same init_params. Root cause: `DataTreatment.get_corrected_sd` in subprocess computes `uv_base_curve.diff_spline` from the FULL raw UV data [0, 2399] (2400 frames), while in-process `get_curve_xy_impl(self.sd, ...)` uses the trimmed legacy SD [0, 633] (634 frames). Same init `params[-1]` applied to the two different splines → completely different UV baseline corrections → 5.5 SV gap. Fix: added export of `uv_diff_spline_x.npy` / `uv_diff_spline_y.npy` from BackRunner's ip_*.npy block (using `uv_base_curve.diff_spline.get_knots()` for x and evaluating at knots for y). Loading code already existed in `OptimizerMain.create_optimizer_from_job` (lines ~188-195). This replaces the subprocess's full-range spline with the in-process's trimmed spline, making both paths numerically identical. The optimizer was already reaching SV=78.72 despite the gap; fix closes the starting SV. |
